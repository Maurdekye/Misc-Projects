...
class Command:
    ...
    def __init__(this, parentcoms):
        this.parentcoms = parentcoms
    
    def __str__(this):
        return this.helptext

    def activate(this, args, dat, caps):
        pass
...
commands += [credit(commands)]
class preload(Command):
    name = 'preload'
    helptext = 'preload [dataset 1] [dataset 2] [etc.] - Preloads all listed data values as the last used dataset, enter it without arguments to clear the last used dataset in memory. Fairly advanced command.'
    def activate(this, args, dat, caps):
        commit = {}
        if len(args) == 0:
            print 'Cleared data in memory.'
        else:
            for a in args:
                if a in dat.values():
                    commit[a] = dat[a]
            print 'Loaded values {} into memory for use.'.format(prettyprint(commit.values()))
...
while True:
    ...
    for c in commands:
        if c.name == com:
            if '-r' in args:
                args.remove('-r')
                c.activate(args, commit, uppers)
            else:
                c.activate(args, data, uppers)
            break
    else:
        print "'{}' is not a valid command.".format(com)
    print
